		...
		const PackedStringArray& p_features, bool p_is_debug, const String& p_path, uint32_t p_flags)
{
	...
	std::map<std::string, String> platform_settings = { { "windows", settings->project_settings.windows_platform_path },
		{ "macos", settings->project_settings.mac_platform_path },
		{ "linux", settings->project_settings.linux_platform_path },
		{ "ios", settings->project_settings.ios_platform_path },
		{ "android", settings->project_settings.android_platform_path } };
	...
	for (const auto& feature : p_features)
	{
		...
		{
			String platform_path = settings->get_setting(it->second);
