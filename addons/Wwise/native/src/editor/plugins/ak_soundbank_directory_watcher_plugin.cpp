...
void AkSoundBankDirectoryWatcherPlugin::init_project_db(const String& p_root_directory_path, uint64_t p_time, String custom_platform_name)
{
	if (p_time > last_soundbank_directory_update)
	{
		last_soundbank_directory_update = p_time;
		WwiseProjectDatabase::get_singleton()->init(p_root_directory_path, AkEditorSettings::get_platform_name(custom_platform_name));
