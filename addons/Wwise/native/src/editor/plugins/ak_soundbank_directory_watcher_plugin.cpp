...
void AkSoundBankDirectoryWatcherPlugin::_on_timer_timeout()
{
	...
	if (root_output_path.is_empty())
	{
		if (!empty_base_path_error_was_logged)
		{
			UtilityFunctions::push_warning(
					"WwiseGodot: The 'Root Output Path' setting in the Wwise 'Common User Settings' (found in the "
					"Godot Project Settings) is empty. Please generate the SoundBanks in the Godot project and set the "
					"'Root Output Path' to complete the setup process.");
	...
	if ((Time::get_singleton()->get_unix_time_from_system() - last_file_check) > s_between_checks)
	{
		...
		task = std::async(std::launch::async,
				[this, root_output_path, time]() { this->init_project_db(root_output_path, time); });
...
void AkSoundBankDirectoryWatcherPlugin::init_project_db(const String& p_root_directory_path, uint64_t p_time)
{
	if (p_time > last_soundbank_directory_update)
	{
		last_soundbank_directory_update = p_time;
		WwiseProjectDatabase::get_singleton()->init(p_root_directory_path, AkEditorSettings::get_platform_name());
