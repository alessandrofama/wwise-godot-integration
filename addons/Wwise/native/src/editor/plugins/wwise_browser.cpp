...
bool WwiseBrowser::generate_soundbanks()
{
	...
	Dictionary response = waapi->client_call_with_timeout(
			"ak.wwise.ui.commands.execute", JSON::stringify(args), JSON::stringify(options), 10);
...
String WwiseBrowser::sanitize_name(const String& p_name) { return p_name.replace("-", "_").replace(" ", "_"); }
...
void WwiseBrowser::_on_generate_ids_button_up()
{
	...
	generate_ids("AUX_BUSSES", WwiseProjectInfo::get_singleton()->get_data()->get_bus_root());
