...
private:
	inline static Ref<Texture2D> cached_icons[AK_ICON_LENGTH]{};
...
public:
	...
	static constexpr AkEditorIconType get_ak_editor_icon_type(const WwiseObjectType type)
	{
		switch (type)
		{
			case AKTYPE_EVENT:
				return AK_ICON_EVENT;
			case AKTYPE_SOUNDBANK:
	...
	static Ref<Texture2D> get_editor_icon(const WwiseObjectType ak_type);
	static Ref<Texture2D> get_editor_icon(const AkEditorIconType icon_type);
};