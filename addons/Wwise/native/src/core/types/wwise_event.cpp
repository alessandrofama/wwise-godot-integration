...
void WwiseEvent::post_load_auto_bank(uint32_t p_bank_id)
{
	...
	const char* event_name = get_name().utf8().get_data();
	auto result = AK::SoundEngine::PrepareEvent(AK::SoundEngine::Preparation_Load, &event_name, 1);
...
void WwiseEvent::unload_auto_bank()
{
	...
	{
		const char* event_name = get_name().utf8().get_data();
		AK::SoundEngine::PrepareEvent(AK::SoundEngine::Preparation_Unload, &event_name, 1);
